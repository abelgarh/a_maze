...
class Maze:
    ...
    def generate(self, seed: int) -> None:
        ...
        exit_cel = self.get_cell(self.exit_pos[1], self.exit_pos[0])
        exit_cel.visited = False
    ...
    def shortest_path(self) -> str:
        ...
        """
        start_cell = self.get_cell(self.entry_pos[1], self.entry_pos[0])
        exit_cell = self.get_cell(self.exit_pos[1], self.exit_pos[0])

        queue = [(start_cell, "")]

        visited_bfs = set()
        visited_bfs.add((start_cell.row, start_cell.col))

        while queue:
            curr_cell, path = queue.pop(0)

            if curr_cell == exit_cell:
                return path

            if not curr_cell.north:
                nr, nc = curr_cell.row - 1, curr_cell.col
                if self.is_in_bounds(nr, nc) and (nr, nc) not in visited_bfs:
                    visited_bfs.add((nr, nc))
                    queue.append((self.get_cell(nr, nc), path + "N"))

            if not curr_cell.south:
                nr, nc = curr_cell.row + 1, curr_cell.col
                if self.is_in_bounds(nr, nc) and (nr, nc) not in visited_bfs:
                    visited_bfs.add((nr, nc))
                    queue.append((self.get_cell(nr, nc), path + "S"))

            if not curr_cell.east:
                nr, nc = curr_cell.row, curr_cell.col + 1
                if self.is_in_bounds(nr, nc) and (nr, nc) not in visited_bfs:
                    visited_bfs.add((nr, nc))
                    queue.append((self.get_cell(nr, nc), path + "E"))

            if not curr_cell.west:
                nr, nc = curr_cell.row, curr_cell.col - 1
                if self.is_in_bounds(nr, nc) and (nr, nc) not in visited_bfs:
                    visited_bfs.add((nr, nc))
                    queue.append((self.get_cell(nr, nc), path + "W"))
    ...
    def not_perfect(self) -> None:
        ...
        while broke_wal < numb_of_wal_break:
            ...
            if direction == "north" and cell.north:
                self.remove_wall_between(cell, self.get_cell(r - 1, c))
                broke_wal += 1
            elif direction == "east" and cell.east:
                self.remove_wall_between(cell, self.get_cell(r, c + 1))
                broke_wal += 1
...
def shortest_path(self, start: tuple[int, int], exit_: tuple[int, int]) -> list[tuple[int, int]]:
    queue = deque([start])
    visited = set([start])
    parent = {}
